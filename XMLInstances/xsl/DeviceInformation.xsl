...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:include href="AlternateNames.xsl" />
			...
			<td>
				<xsl:choose>
					<xsl:when test="sgr:isLocalControl = 'true'">
						<img src="/xsl/lan.png" alt="" width="16pt" height="16pt" /> Local area </xsl:when>
					<xsl:otherwise>
						<img src="/xsl/cloud.png" alt="" width="16pt" height="16pt" /> Cloud device </xsl:otherwise>
				</xsl:choose>
			</td>
		</tr>
			...
			<td>
				<xsl:choose>
					<xsl:when test="sgr:transportService='Modbus'">
						<img src="/xsl/modbus.png" alt="Modbus" height="17px" />
					</xsl:when>
					<xsl:when test="sgr:transportService='RESTfulJSON'">
						<img src="/xsl/rest.png" alt="JsonRest" height="17px" />
					</xsl:when>
					<xsl:when test="sgr:transportService='Contacts'">
						<img src="/xsl/contacts.png" alt="Contacts" height="17px" />
					</xsl:when>
					<xsl:otherwise>
						<!--enumeration
						value="EEBUS"/>
